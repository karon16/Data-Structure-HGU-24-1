...
tree_node* BinTree::search(int key) {
    tree_node *tree = root;
    while(tree){
        if(key == tree->data.key) return nullptr; // key is present
        else if(key < tree->data.key)
            if(tree->left_child == nullptr) return tree;
            else tree = tree->left_child;
        else // key > tree->key
        if(tree->right_child == nullptr) return tree;
        else tree = tree->right_child;
    }
    return nullptr;
}

tree_node* BinTree::search_to_delete(int key) {
    tree_node *tree = root;
    while (tree) {
...
void BinTree::insert_node(node_data data) {
